...
export class File {
  ...
  public config(param: Boolean) {
    try {
      let set: String = fs.readFileSync(
  ...
  public save(message: String = "", num: number = 0) {
    ...
    if (this.daly) {
      let date = new Date();
      pathDirectory += `\\${date.getFullYear()}-${
        date.getMonth() + 1
      }-${date.getDate()}`;
    }
    ...
    if (message === "") {
      ...
      concatName = `${pathDirectory}\\${splitName}-${date2.getFullYear()}-${
        date2.getMonth() + 1
      }-${date2.getDate()}-${date2.getHours()}h${date2.getMinutes()}m${date2.getSeconds()}s${ext}`;
    } else {
      if (notError) {
        //personal message
  ...
  public slvCopyUpdate(concatName: string, num: number) {
    let nameFile: string =
      this.pathFile.split("\\")[this.pathFile.split("\\").length - 1];
    try {
      let slvJson = JSON.parse(
        fs.readFileSync(this.folder + "/SLVcopy-lock.json", "utf-8")
      );
      ...
      fs.writeFileSync(
        this.folder + "/SLVcopy-lock.json",
        JSON.stringify(slvJson)
      );
    } catch {
      let insert = [
        {
