...
export class Message {
  ...
  public error() {
    window.showErrorMessage(
      "Backup failed. The destination folder is not accessible"
    );
  }
  /*END error()*/
  /**error()
   ...
   *@retrun {void}
   */
  public errorUNC(server: string) {
    window.showErrorMessage(
      `Backup failed. The UNC host '${server}' acces is not allowed, please allow UNChost '${server}' in settings`
    );
  }
  /*END error()*/
  /**showInputBox()
  ...
  public showInputBox() {
    return window.showInputBox({
      prompt:
        "Type the title of your backup. Be careful, do not put punctuation",
